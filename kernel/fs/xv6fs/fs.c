...
xv6fs_iupdate(struct inode *ip)
{
  struct buf *bp;
  struct dinode *dip;
  struct xv6fs_inode* xv6_ip = ip->private;
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
  dip = (struct dinode*)bp->data + ip->inum%IPB;
  dip->type = ip->type;
  dip->major = xv6_ip->major;
  dip->minor = xv6_ip->minor;
  dip->nlink = ip->nlink;
  dip->size = ip->size;
  memmove(dip->addrs, xv6_ip->addrs, sizeof(xv6_ip->addrs));
  bwrite(bp);
  brelse(bp);
}
...
int
xv6fs_readi(struct inode *ino, char user_dst, uint64 dst, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;
...
int
xv6fs_writei(struct inode *ino, char user_src, uint64 src, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;
  struct xv6fs_inode* ip=ino->private;
...
xv6fs_create(struct inode *dp, struct dentry *target, short type, short major, short minor)
{
  struct inode *ip;
  char name[DIRSIZ];
  strncpy((char*)(target->private), name, MAXPATH);
